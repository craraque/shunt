...
/// which can't reach a VM or upstream reachable only through a non-default interface.
...
final class POSIXTCPClient {
    ...
    private func doConnect(host: String, port: UInt16, bindInterface: String?, completion: @escaping (Error?) -> Void) {
        ...
        // Bind to the requested interface and avoid NECP default-interface scoping — NE providers' outbound sockets are scoped to the primary NIC
        // by default, which blocks reaching Parallels  (10.211.55.x). Per Quinn
