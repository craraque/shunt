...
**Goal:** Validate whether Shunt can correctly route only selected macOS app traffic through a local test proxy, without using Zscaler.
...
This plan is for **validation**, not production hardening. It deliberately avoids Zscaler so we can isolate Shunt behavior from corporate VPN/client behavior.

## Locality / Git Safety

- Any code changes Hermes makes stay local in `/Users/admin/projects/shunt` unless explicitly asked to push.
...
- Closer to final Zscaler-in-VM architecture.
...
6. Only after that, test with Zscaler/VM.
